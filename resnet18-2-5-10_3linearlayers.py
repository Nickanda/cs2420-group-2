...
def train_gating_network(gating_net, students, data_loader, device, epochs=5):
    optimizer = AdamW(gating_net.parameters(), lr=config.lr)
    criterion = nn.CrossEntropyLoss()
    gating_net.train()
    ...
    for epoch in range(epochs):
        ...
        for inputs, targets in data_loader:
            ...
            # Calculate each student's output
            student_outputs = torch.stack([student(inputs) for student in students], dim=1)

            # Gating probabilities
            gating_probs = gating_net(inputs.view(inputs.size(0), -1))

            # Weighted sum of student outputs
            moe_output = torch.einsum('bi,bio->bo', gating_probs, student_outputs)

            loss = criterion(moe_output, targets)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()

            total_loss += loss.item()
            correct += (moe_output.argmax(dim=1) == targets).sum().item()

        accuracy = correct / len(data_loader.dataset)
        print(f"Gating Network Epoch {epoch+1}: Loss {total_loss:.4f}, Accuracy {accuracy:.4f}")


def main():
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

    # Get data loaders for students
    train_loader, test_loader, train_loaders = get_data_loaders(num_students=config.num_students, classes_per_group=5)

    # Load teacher and train
    ...
    train_gating_network(gating_net, students, train_loader, device, epochs=5)

    # Combine into MoE and evaluate
